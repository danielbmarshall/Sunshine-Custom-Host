...
from misc.constants import SUNSHINE_TOOLS_DIR
...
class DownloadManager:
    ...
    def _ensure_sunshine_tools_directory(self) -> bool:
        """
        Make sure the Sunshine tools directory exists.
        """
        try:
            os.makedirs(SUNSHINE_TOOLS_DIR, exist_ok=True)
            return True
        except Exception as e:
            log_error(f"Unable to create Sunshine tools directory ({SUNSHINE_TOOLS_DIR}): {e}")
            return False

    def _relocate_portable_tool(self, source_path: str, destination_path: str, tool_name: str) -> bool:
        """
        Move an extracted portable tool to its final destination.
        """
        if not source_path or not os.path.exists(source_path):
            log_error(f"{tool_name} download did not produce the expected files.")
            return False

        if not self._ensure_sunshine_tools_directory():
            return False

        try:
            os.makedirs(os.path.dirname(destination_path), exist_ok=True)
        except OSError:
            # Parent already exists or cannot be created; continue and let move raise an error if needed
            pass

        try:
            if os.path.exists(destination_path):
                log_info(f"Removing existing {tool_name} directory at {destination_path}")
                shutil.rmtree(destination_path)
            log_progress(f"Moving {tool_name} files to {destination_path}")
            shutil.move(source_path, destination_path)
            log_success(f"{tool_name} installed to {destination_path}")
            return True
        except Exception as e:
            log_error(f"Failed to prepare {tool_name}: {e}")
            return False

    def _download_portable_tool(self, config_key: str, display_name: str, tracker_key: str, install: bool = True, selective: bool = False):
        """
        Generic handler to download and optionally install a portable tool.
        """
        tool_config = self.sr.all_configs.get(config_key)
        if not tool_config:
            log_error(f"Missing configuration for {display_name}.")
            if selective:
                self.sr.pause()
            return
        ...
        download_pattern = tool_config.get("download_pattern")

        extracted_path = self._download_file(download_url, download_pattern)
        ...
        if not install:
            log_success(f"{display_name} downloaded to {extracted_path}")
            if selective:
                self.sr.pause()
            return

        destination_dir = tool_config.get("install_dir") or os.path.join(SUNSHINE_TOOLS_DIR, display_name.replace(" ", ""))
        if not self._relocate_portable_tool(extracted_path, destination_dir, display_name):
            if selective:
                self.sr.pause()
            return
        ...
        try:
            install_info = {
                ...
                "files_created": [destination_dir],
                "custom_options": {
                    "download_url": download_url,
                    "pattern": download_pattern,
                    "installation_method": "Portable archive deployment",
                    "destination": destination_dir
                }
            }
            self._tracker.track_installation(tracker_key, destination_dir, install_info)
            log_info(f"{display_name} installation tracked for future uninstallation")
        except Exception as e:
            log_warning(f"Could not track {display_name} installation: {e}")
    ...
    def download_all(self, install: bool = True):
        ...
        try:
            ...
            components = [
                ...
                ("AdvancedRun", self.download_advanced_run),
                ("AutoHideMouseCursor", self.download_auto_hide_mouse_cursor)
            ]
    ...
    def download_advanced_run(self, install: bool = True, selective: bool = False):
        self._download_portable_tool("AdvancedRun", "AdvancedRun", "advanced_run", install, selective)

    def download_auto_hide_mouse_cursor(self, install: bool = True, selective: bool = False):
        self._download_portable_tool("AutoHideMouseCursor", "AutoHideMouseCursor", "auto_hide_mouse_cursor", install, selective)
