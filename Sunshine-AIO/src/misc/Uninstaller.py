...
class SunshineAIOUninstaller:
    ...
    def __init__(self, system_requests: SystemRequests) -> None:
        ...
        self._components = {
            ...
            "advanced_run": {
                "name": "AdvancedRun",
                "uninstaller_method": self._uninstall_advanced_run_advanced,
                "verification_method": self._verify_advanced_run_uninstalled,
                "processes": ["AdvancedRun.exe"]
            },
            "auto_hide_mouse_cursor": {
                "name": "AutoHideMouseCursor",
                "uninstaller_method": self._uninstall_auto_hide_mouse_cursor_advanced,
                "verification_method": self._verify_auto_hide_mouse_cursor_uninstalled,
                "processes": ["AutoHideMouseCursor.exe"]
            }
        }
    ...
    def _uninstall_advanced_run_advanced(self) -> bool:
        """
        Advanced uninstallation for Nirsoft AdvancedRun.
        """
        log_info("Starting AdvancedRun uninstallation...")
        success = True

        try:
            if not self._stop_processes(["AdvancedRun.exe"]):
                success = False

            files_to_remove = self._tracker.get_files_to_remove("advanced_run")
            if files_to_remove and not self._remove_files_and_directories(files_to_remove):
                success = False

            if success:
                log_success("AdvancedRun uninstalled successfully")
            else:
                log_warning("AdvancedRun uninstallation completed with warnings")

            return success
        except Exception as e:
            log_error(f"Error during AdvancedRun uninstallation: {e}")
            return False

    def _verify_advanced_run_uninstalled(self) -> bool:
        """
        Ensure AdvancedRun files are gone.
        """
        for path in self._tracker.get_all_installation_paths("advanced_run"):
            if os.path.exists(path):
                return False
        return True

    def _uninstall_auto_hide_mouse_cursor_advanced(self) -> bool:
        """
        Advanced uninstallation for AutoHideMouseCursor.
        """
        log_info("Starting AutoHideMouseCursor uninstallation...")
        success = True

        try:
            if not self._stop_processes(["AutoHideMouseCursor.exe"]):
                success = False

            files_to_remove = self._tracker.get_files_to_remove("auto_hide_mouse_cursor")
            if files_to_remove and not self._remove_files_and_directories(files_to_remove):
                success = False

            if success:
                log_success("AutoHideMouseCursor uninstalled successfully")
            else:
                log_warning("AutoHideMouseCursor uninstallation completed with warnings")

            return success
        except Exception as e:
            log_error(f"Error during AutoHideMouseCursor uninstallation: {e}")
            return False

    def _verify_auto_hide_mouse_cursor_uninstalled(self) -> bool:
        """
        Ensure AutoHideMouseCursor files are gone.
        """
        for path in self._tracker.get_all_installation_paths("auto_hide_mouse_cursor"):
            if os.path.exists(path):
                return False
        return True
